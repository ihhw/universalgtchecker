/**
 * Server-side proxy pool for the Discord checker.
 *
 * Discord's unauthenticated username endpoint rate-limits a single IP hard.
 * Proxies are optional: without any, checks go out directly from this
 * server's own IP and the rate is kept low (see MAX_RATE_NO_PROXY in
 * discord-availability.ts). With proxies, load spreads across them and a
 * much higher rate becomes usable.
 *
 * Proxy URLs can carry credentials, so — like the Discord webhook URL —
 * they are stored only on the server and never returned to the browser;
 * the API exposes just a count and a masked preview.
 */
...
const FILE = path.join(process.cwd(), "discord-proxies.json");
...
function persist(next: Stored): void {
  ...
  try {
    ...
    logger.warn({ err }, "Could not persist Discord proxy settings");
  }
}
