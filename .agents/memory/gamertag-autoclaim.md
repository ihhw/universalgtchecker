---
name: Gamertag Auto-Claim Endpoint
description: How claiming works (reserve → change with the xboxlive.com XSTS token), and the two wrong change endpoints already ruled out by live testing
---

## Flow (lib/xbox-claim.ts — the only claim path)
1. `POST https://gamertag.xboxlive.com/gamertags/reserve` `{ classicGamertag, reservationId: <xuid>, targetGamertagFields: "classicGamertag" }`
   — **CONFIRMED against live Xbox**: this call is accepted (reserve succeeds; the account's own auth/XUID chain is correct).
2. `PUT https://accounts.xboxlive.com/users/current/profile/gamertag` `{ gamertag, previewOnly: false }`, `x-xbl-contract-version: 3`
   — current best guess (OpenXbox `xbox-webapi` / reverse-engineered Xbox app traffic). **Not yet confirmed against live Xbox.**

Both use `Authorization: XBL3.0 x={uhs};{xsts}` where the XSTS relying party is `http://xboxlive.com`.

## Endpoints already ruled out by real testing — do not reintroduce
- `PUT gamertag.xboxlive.com/users/xuid({xuid})/gamertag` — confirmed **HTTP 404** against live Xbox
  (user report, 2026-09-23: reserve succeeded in 2214ms, then this URL 404'd). This was the very first guess.
- The original pre-fix code sent an `http://accounts.xboxlive.com`-scoped XSTS token to `gamertag.xboxlive.com`
  (wrong audience → 401/403) and PUT to other guessed URLs with no reservation at all. Browser-side auto-claim
  also claimed every hit and only ran with the tab open.

If the current `accounts.xboxlive.com/users/current/profile/gamertag` endpoint also turns out wrong, the next
thing to try (in order of how often it's cited in reverse-engineering write-ups) is the same URL with
`x-xbl-contract-version: 2` instead of `3`, or a body shaped `{ Gamertag, PreviewOnly }` (capitalized keys).
Get the exact response (status + body) from the user before guessing again — Xbox's error body usually names
the real reason (e.g. code 1441 = no free gamertag change left on the account).

## Success rule
CLAIMED only when the change response names the exact tag with no suffix, or a force-refreshed XSTS `gtg` claim equals the tag. 2xx without proof → UNKNOWN. 5xx/timeout at change → confirm via identity before deciding. One claim at a time (global lock).

## Uncertainty
These endpoints are those of Xbox's own apps' gamertag-change flow, not officially documented by Microsoft, and
Xbox itself is unreachable from the dev sandbox (network policy blocks `*.xboxlive.com`), so behaviour here is
tested against `test/mock-xbox.ts`, not live Xbox. The reserve step above is the one piece confirmed live so
far, by the user's own test run.
